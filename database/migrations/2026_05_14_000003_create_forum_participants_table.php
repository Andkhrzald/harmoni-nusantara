<?php

use Illuminate\Database\Migrations\Migration;
use Illuminate\Database\Schema\Blueprint;
use Illuminate\Support\Facades\Schema;

return new class extends Migration
{
    public function up(): void
    {
        Schema::create('forum_participants', function (Blueprint $table) {
            $table->id();
            $table->foreignId('forum_room_id')->constrained()->cascadeOnDelete();
            $table->foreignId('user_id')->constrained()->cascadeOnDelete();
            $table->string('role', 20)->default('viewer'); // creator, member, viewer
            $table->string('status', 20)->default('pending'); // active, pending, banned
            $table->timestamps();

            $table->unique(['forum_room_id', 'user_id']);
        });
    }

    public function down(): void
    {
        Schema::dropIfExists('forum_participants');
    }
};
